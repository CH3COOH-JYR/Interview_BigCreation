...
// AI 模型配置 - 使用第三方API服务
const API_URL = process.env.AI_API_URL || "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions";
const MODEL_NAME = process.env.AI_MODEL_NAME || "qwen-plus"; // 切换到非推理模型，速度更快
const API_KEY = process.env.AI_API_KEY || "sk-12f269040305482b9e0c756f0a65c171"; // 临时API Key
...
// 生成带上下文的过渡语句（改进版）
exports.generateTransitionWithContext = async (previousQuestion, nextQuestion, dialogContext, currentIndex, totalQuestions) => {
  const messages = [
    {
      role: "system",
      content: (
        "您是一位专业的访谈主持人。请根据访谈的上下文和进度，生成自然、个性化的过渡语句。"
        + "过渡语句应该：\n"
        + "1. 承接之前的对话内容\n"
        + "2. 自然引入下一个话题\n"
        + "3. 保持访谈的连贯性和温度\n"
        + "4. 根据访谈进度调整语调（开始、中间、接近结束）\n"
        + "只输出过渡语句本身，不要包含问题内容。"
      )
    },
    {
      role: "user", 
      content: (
        `访谈进度: 第${currentIndex + 1}/${totalQuestions}个问题\n`
        + `上一话题: ${previousQuestion || '开场'}\n`
        + `下一话题: ${nextQuestion}\n`
        + `最近对话:\n${dialogContext}\n\n`
        + "请生成一个自然、个性化的过渡语句，要体现对受访者前面回答的关注和对下一话题的引导。"
      )
    }
  ];
  ...
  if (!result) {
    // 根据访谈进度生成不同的默认过渡语句
    const progressRatio = currentIndex / totalQuestions;
    if (progressRatio < 0.3) {
      return '很好，接下来让我们探讨另一个重要话题。';
    } else if (progressRatio < 0.7) {
      return '感谢您的深入分享，现在我想了解一下';
    } else {
      return '我们的访谈接近尾声，最后我想请您谈谈';
    }
  }
...
exports.checkOffTopicAndGuide = async (currentQuestion, userResponse) => {
  const messages = [
    {
      role: "system",
      content: (
        "你是一位经验丰富的访谈员，需要帮助受访者更好地理解和回答问题。当发现回答偏题时，你需要：\n"
        + "1. 分析问题的核心要点和深层含义\n"
        + "2. 理解受访者回答偏离的具体方向\n"
        + "3. 基于以上两点，生成富有洞察力的引导\n\n"
        + "如果回答明显偏题，返回JSON格式：\n"
        + "{\"isOffTopic\": true, \"guidance\": \"引导文字\"}\n"
        + "引导文字要求：\n"
        + "- 首先肯定受访者已有的回答中有价值的部分\n"
        + "- 然后指出问题更深层的探讨方向\n"
        + "- 可以通过举例或具体场景来阐明问题的重点\n"
    ...
    },
    {
      ...
      content: (
        `当前问题: ${currentQuestion}\n`
        + `受访者回答: ${userResponse}\n\n`
        + "请分析这个回答是否偏离了问题主题。如果偏题，请生成一段有见地的引导，"
        + "帮助受访者理解问题的深层含义并引导其分享更有价值的观点。"
      )
    }
  ];
  ...
  try {
    ...
    try {
      ...
      const isOffTopic = isOffTopicKeywords.some(keyword => cleanedResult.toLowerCase().includes(keyword));
      
      return {
        isOffTopic: isOffTopic,
        guidance: isOffTopic ? `让我重新解释一下这个问题：${currentQuestion}。请您结合这个问题的具体内容，分享您的观点和经验。` : ''
      };
    }
  } catch (parseError) {
