...
// 确认进入最后一个问题
exports.confirmLastQuestion = async (req, res) => {
  try {
    const result = await interviewService.confirmLastQuestion(req.params.id);
    ...
    console.error('Error confirming last question:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
